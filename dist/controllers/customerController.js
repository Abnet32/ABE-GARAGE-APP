...
export const updateCustomer = async (req, res) => {
    try {
        ...
        const customer = await CustomerIdentifier.findByIdAndUpdate(id, { email, phone_number: phone }, { new: true });
        if (!customer)
            return res.status(404).json({ message: "Customer not found" });
        // Find and update CustomerInfo
        const info = await CustomerInfo.findOneAndUpdate({ customer_id: id }, { first_name: firstName, last_name: lastName }, { new: true });
        res.json({
