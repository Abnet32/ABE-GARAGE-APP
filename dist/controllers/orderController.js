...
export const getOrders = async (req, res) => {
    try {
        ...
        const enrichedOrders = await Promise.all(orders.map((order) => formatOrderResponse(order)));
...
export const createOrder = async (req, res) => {
    try {
        const { customer_id, employee_id, vehicle_id, services, total_price, description } = req.body;
...
export const updateOrder = async (req, res) => {
    try {
        ...
        // Update order
        const updated = await Order.findByIdAndUpdate(orderId, { order_status: order_status || "Received" }, { new: true })
            .populate("customer_id")
            .populate("employee_id")
            .populate("vehicle_id");
        if (!updated) {
        ...
        if (order_status === "Completed") {
            await OrderInfo.findOneAndUpdate({ order_id: orderId }, { order_completion_date: new Date() }, { upsert: true });
        }
