...
export const updateService = async (req, res) => {
    try {
        ...
        const updated = await CommonService.findByIdAndUpdate(req.params.id, { service_name, service_description, active }, { new: true });
        if (!updated)
            return res.status(404).json({ message: "Service not found" });
...
export const deleteService = async (req, res) => {
    try {
        const deleted = await CommonService.findByIdAndUpdate(req.params.id, { active: false }, { new: true });
        if (!deleted)
            return res.status(404).json({ message: "Service not found" });
        res
            .status(200)
