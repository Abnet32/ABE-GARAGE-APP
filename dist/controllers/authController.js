...
export const login = async (req, res) => {
    try {
        ...
        const match = await bcrypt.compare(password, empPass.employee_password_hashed);
        if (!match)
            return res.status(400).json({ message: "Invalid email or password" });
        ...
        // 5️⃣ Generate JWT
        const token = jwt.sign({ id: employee._id, role }, process.env.JWT_SECRET || "defaultsecret", { expiresIn: "1d" });
...
export const register = async (req, res) => {
    try {
        ...
        // 6️⃣ Generate JWT
        const token = jwt.sign({ id: employee._id, role: roleDoc.company_role_name }, process.env.JWT_SECRET || "defaultsecret", { expiresIn: "1d" });
        res.status(201).json({
